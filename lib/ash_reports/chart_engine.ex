defmodule AshReports.ChartEngine do
  ...
  def generate_chart_set(chart_configs, %RenderContext{} = context) when is_map(chart_configs) do
    results = Enum.map(chart_configs, fn {key, config} ->
    ...
    if length(errors) > 0 do
      ...
      successful_charts = results |> Enum.map(fn {key, {:ok, chart}} -> {key, chart} end) |> Map.new()
  ...
  defp get_provider(provider_key) do
    case Map.get(@providers, provider_key) do
      nil -> raise ArgumentError, "Unknown chart provider: #{provider_key}. Available: #{Map.keys(@providers) |> Enum.join(", ")}"
  ...
  defp build_chart_specification(%ChartConfig{} = config, %ChartData{} = data, %RenderContext{} = context) do
    base_spec = %{
      type: config.type,
  ...
  defp suggest_charts_for_data(%{data_type: :numeric, has_time_series: true}, _context) do
    [
      %ChartConfig{type: :line, confidence: 0.9, reasoning: "Time series data best shown as line chart"},
      %ChartConfig{type: :area, confidence: 0.7, reasoning: "Area chart good for cumulative time series"}
    ]
  end
  
  defp suggest_charts_for_data(%{data_type: :categorical, value_count: count}, _context) when count <= 10 do
    [
      %ChartConfig{type: :pie, confidence: 0.8, reasoning: "Small categorical dataset good for pie chart"},
      %ChartConfig{type: :bar, confidence: 0.9, reasoning: "Bar chart excellent for categorical comparisons"}
    ]
  end
  
  defp suggest_charts_for_data(%{data_type: :categorical}, _context) do
    [
      %ChartConfig{type: :bar, confidence: 0.9, reasoning: "Bar chart best for large categorical datasets"}
    ]
  end
  
  defp suggest_charts_for_data(%{data_type: :numeric, numeric_range: {min, max}}, _context) when max - min > 1000 do
    [
      %ChartConfig{type: :histogram, confidence: 0.8, reasoning: "Large numeric range good for histogram"},
      %ChartConfig{type: :boxplot, confidence: 0.7, reasoning: "Box plot shows distribution characteristics"}
    ]
  end
  ...
  defp merge_chart_options(config_options, %RenderContext{} = context) do
    ...
    rtl_options = if context.text_direction == "rtl" do
      %{
        scales: %{
          x: %{reverse: true},
          y: %{position: "right"}
  ...
  defp apply_persian_formatting(chart_spec) do
    ...
    |> put_in([:options, :scales, :y, :ticks, :callback], "function(value) { return value.toLocaleString('fa-IR'); }")
  end
  
  defp apply_urdu_formatting(chart_spec) do
    chart_spec
    |> put_in([:options, :scales, :x, :ticks, :font, :family], "Noto Sans Urdu, Arial, sans-serif")
