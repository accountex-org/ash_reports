defmodule AshReports.LiveView.DashboardPresence do
  ...
  @presence_timeout 300_000  # 5 minutes
  ...
  def track_user(dashboard_id, user_id, user_metadata \\ %{}) do
    ...
    presence_metadata = Map.merge(user_metadata, %{
    ...
    case track(topic, user_id, presence_metadata) do
      ...
      {:error, reason} ->
        Logger.error("Failed to track user #{user_id} in dashboard #{dashboard_id}: #{inspect(reason)}")
  ...
  def update_user_presence(dashboard_id, user_id, updates) do
    ...
    case get_current_presence(topic, user_id) do
      %{metas: [current_meta | _]} ->
        updated_meta = Map.merge(current_meta, Map.merge(updates, %{
          last_activity: DateTime.utc_now()
        }))
      ...
      _ ->
        Logger.warn("Cannot update presence - user #{user_id} not found in dashboard #{dashboard_id}")
        :ok
    end
  end
  ...
  defp broadcast_user_joined(dashboard_id, user_id, user_metadata) do
    ...
    activity_topic = build_activity_topic(dashboard_id)
    Phoenix.PubSub.broadcast(AshReports.PubSub, activity_topic, {:collaboration_event, join_event})
  end
  ...
  defp resolve_simultaneous_edit_conflict(dashboard_id, conflict_info) do
    ...
    winner_user = List.last(users)  # Most recent user
    ...
    activity_topic = build_activity_topic(dashboard_id)
    Phoenix.PubSub.broadcast(AshReports.PubSub, activity_topic, {:conflict_resolution, conflict_resolution})
  ...
  defp resolve_concurrent_filter_conflict(dashboard_id, conflict_info) do
    ...
    merged_filters = conflict_info.filters
