defmodule AshReports.LiveView.DashboardLive do
  ...
  def handle_event("export_dashboard", %{"format" => format}, socket) do
    ...
    case export_dashboard_data(socket, format) do
      {:ok, export_data} ->
        ...
        :ok = push_event(socket, "download_ready", %{
  ...
  def handle_info({:chart_interaction, chart_id, interaction_data}, socket) do
    ...
    case interaction_data.type do
      :filter_change ->
        ...
        updated_filters = merge_chart_filter_to_global(socket.assigns.global_filters, chart_id, interaction_data.filter)
      ...
      :data_highlight ->
        ...
        :ok = broadcast_data_highlight_to_charts(socket, interaction_data.data_point, exclude: chart_id)
        {:noreply, socket}
  ...
  defp parse_dashboard_config(params, session) do
    %{
      dashboard_id: params["dashboard_id"] || session["dashboard_id"] || generate_dashboard_id(),
      title: params["title"] || session["dashboard_title"],
      layout: params["layout"] || "grid",
      real_time: params["real_time"] == "true" || session["real_time"] == true,
      collaboration: params["collaboration"] == "true" || session["collaboration"] == true,
  ...
  defp setup_collaboration_features(socket) do
    ...
    {:ok, _ref} = Phoenix.Presence.track(
      AshReports.Presence,
  ...
  defp broadcast_data_highlight_to_charts(socket, data_point, opts \\ []) do
    ...
    |> Enum.filter(&(&1 != exclude_chart))
    |> Enum.each(fn chart_id ->
      :ok = push_event(socket, "highlight_data", %{
        chart_id: chart_id,
  ...
  defp dashboard_container_classes(dashboard_config, user_info) do
    ...
    feature_classes = if dashboard_config.real_time, do: ["real-time" | feature_classes], else: feature_classes
    feature_classes = if dashboard_config.collaboration, do: ["collaborative" | feature_classes], else: feature_classes
  ...
  defp render_user_avatars(connected_users) do
    ...
    <div class="user-avatars">
      #{Enum.map_join(user_list, "", fn {_user_id, user_info} ->
        "<div class=\"user-avatar\" title=\"#{user_info.name}\">
  ...
  defp build_render_context(assigns) do
    %RenderContext{
      ...
      text_direction: if(assigns.user_info.locale in ["ar", "he", "fa", "ur"], do: "rtl", else: "ltr"),
      metadata: %{
