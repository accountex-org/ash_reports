defmodule AshReports.LiveView.ChartConfigurationComponent do
  ...
  def update(assigns, socket) do
    ...
    socket = if socket.assigns.show_preview do
  ...
  def handle_event("update_config", %{"config" => config_params}, socket) do
    ...
    # Update preview if enabled
    socket = if socket.assigns.preview_enabled do
      update_chart_preview(socket)
    else
      socket
    end
  ...
  defp validate_chart_configuration(socket) do
    ...
    # Validate title
    errors = if is_binary(config[:title]) and String.trim(config[:title]) == "" do
      Map.put(errors, :title, get_title_required_error(socket.assigns.locale))
    else
      errors
    end
    
    # Validate data source
    errors = case config[:data_source_type] do
      "database" ->
        if is_binary(config[:database_query]) and String.trim(config[:database_query]) == "" do
          Map.put(errors, :database_query, get_query_required_error(socket.assigns.locale))
        else
          errors
        end
      
      "api" ->
        if not valid_url?(config[:api_endpoint]) do
          Map.put(errors, :api_endpoint, get_valid_url_required_error(socket.assigns.locale))
        else
          errors
        end
    ...
    # Validate update interval for real-time charts
    errors = if config[:real_time] and not valid_update_interval?(config[:update_interval]) do
      Map.put(errors, :update_interval, get_valid_interval_error(socket.assigns.locale))
    else
      errors
    end
  ...
  defp update_chart_preview(socket) do
    if socket.assigns.configuration_valid do
      ...
      preview_config = %ChartConfig{
        ...
        interactive: false,  # Disable interactions in preview
        real_time: false    # Disable real-time in preview
      }
  ...
  defp valid_update_interval?(interval) when is_integer(interval) do
    interval >= 1000 and interval <= 300_000  # Between 1 second and 5 minutes
  end
  ...
  defp get_invalid_config_error_text("fr"), do: "Veuillez corriger les erreurs avant d'enregistrer"
