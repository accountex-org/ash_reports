defmodule AshReports.LiveView.AccessControl do
  ...
  """
  @spec authorize_dashboard_access(String.t(), String.t(), atom()) ::
  ...
  def authorize_dashboard_access(user_id, dashboard_id, action) do
    ...
    case has_permission?(user_id, required_permission, dashboard_id) do
      ...
      false ->
        ...
        {:unauthorized,
         "User #{user_id} lacks permission #{required_permission} for dashboard #{dashboard_id}"}
    end
  end
  ...
  """
  @spec authorize_chart_access(String.t(), String.t(), String.t(), atom()) ::
          :authorized | {:unauthorized, String.t()}
  def authorize_chart_access(user_id, dashboard_id, chart_id, action) do
    ...
    case has_permission?(user_id, required_permission, dashboard_id) do
      ...
      false ->
        ...
        {:unauthorized,
         "User #{user_id} lacks permission #{required_permission} for chart #{chart_id}"}
    end
  end
  ...
  def handle_call({:grant_permission, user_id, permission, resource_id}, _from, state) do
    ...
    if permission in current_permissions do
      ...
      audit_entry =
        create_audit_entry(user_id, resource_id, :permission_granted, %{permission: permission})

      updated_audit = [audit_entry | state.audit_log]

      Logger.debug(
        "Granted permission #{permission} to user #{user_id} for resource #{resource_id}"
      )
  ...
  def handle_call({:revoke_permission, user_id, permission, resource_id}, _from, state) do
    ...
    audit_entry =
      create_audit_entry(user_id, resource_id, :permission_revoked, %{permission: permission})

    updated_audit = [audit_entry | state.audit_log]

    Logger.debug(
      "Revoked permission #{permission} from user #{user_id} for resource #{resource_id}"
    )
  ...
  defp map_action_to_permission(action, resource_type) do
    case resource_type do
      :dashboard -> map_dashboard_action_to_permission(action)
      :chart -> map_chart_action_to_permission(action)
      _ -> map_general_action_to_permission(action)
    end
  end

  defp map_dashboard_action_to_permission(action) do
    case action do
      :view -> :view_dashboard
      :edit -> :edit_dashboard
  ...
  defp map_chart_action_to_permission(action) do
    case action do
      :view -> :view_charts
      :interact -> :interact_charts
      :edit -> :edit_charts
  ...
  defp map_general_action_to_permission(action) do
    case action do
      :export -> :export_data
      :share -> :share_dashboard
      :real_time -> :real_time_access
      _ -> :view_dashboard
    end
  end
  ...
  defp log_access_attempt(user_id, dashboard_id, action, result) do
    Logger.debug(
      "Access attempt: user=#{user_id}, dashboard=#{dashboard_id}, action=#{action}, result=#{result}"
    )
  ...
  defp log_chart_access_attempt(user_id, dashboard_id, chart_id, action, result) do
    Logger.debug(
      "Chart access: user=#{user_id}, dashboard=#{dashboard_id}, chart=#{chart_id}, action=#{action}, result=#{result}"
    )
  ...
  defp schedule_audit_cleanup do
    # 24 hours
    Process.send_after(self(), :cleanup_audit_log, 24 * 60 * 60 * 1000)
  end

  defp filter_by_user_id(entries, nil), do: entries

  defp filter_by_user_id(entries, user_id) do
  ...
  def require_permission(permission, resource_id_key \\ :dashboard_id) do
    quote do
      def mount(params, session, socket) do
        ...
        case __MODULE__.authorize_dashboard_access(
               user_id,
               resource_id,
               unquote(permission)
             ) do
