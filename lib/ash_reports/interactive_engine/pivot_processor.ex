defmodule AshReports.InteractiveEngine.PivotProcessor do
  ...
  def generate_pivot_table(data, config, %RenderContext{} = context)
    ...
    pivot_with_totals =
      if config.show_subtotals or config.show_grand_total do
  ...
  defp build_pivot_cell(
    ...
    matching_records =
      find_matching_records(data, row_combination, column_combination, row_fields, column_fields)

    aggregated_values = calculate_cell_aggregations(matching_records, value_fields, aggregation)
  ...
  defp find_matching_records(data, row_combination, column_combination, row_fields, column_fields) do
    data
    |> Enum.filter(fn item ->
      row_match = fields_match?(item, row_fields, row_combination)
      column_match = fields_match?(item, column_fields, column_combination)
      row_match and column_match
    end)
  end

  defp fields_match?(item, fields, expected_values) do
    fields
  ...
  defp calculate_cell_aggregations(matching_records, value_fields, aggregation) do
    value_fields
  ...
  defp add_totals(pivot_data, _row_values, _column_values, config, %RenderContext{} = context) do
    pivot_with_subtotals =
      if config.show_subtotals do
  ...
  defp add_row_subtotals(pivot_data, _config) do
    # Add subtotals for each row
    Enum.map(pivot_data, fn row_data ->
      subtotal_values = calculate_row_subtotals(row_data.columns)
      subtotal_column = build_subtotal_column(row_data, subtotal_values)
      %{row_data | columns: row_data.columns ++ [subtotal_column]}
    end)
  end

  defp calculate_row_subtotals(columns) do
    columns
    |> Enum.reduce(%{}, fn column_data, acc ->
      accumulate_column_values(column_data.values, acc)
    end)
  end

  defp accumulate_column_values(column_values, accumulator) do
    Enum.reduce(column_values, accumulator, fn {field, value}, field_acc ->
      Map.update(field_acc, field, value, &(&1 + value))
    end)
  end

  defp build_subtotal_column(row_data, subtotal_values) do
    %{
      row: row_data.row,
      column: ["SUBTOTAL"],
      values: subtotal_values,
      record_count: Enum.map(row_data.columns, & &1.record_count) |> Enum.sum()
    }
  end
