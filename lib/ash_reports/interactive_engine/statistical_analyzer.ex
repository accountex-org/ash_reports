defmodule AshReports.InteractiveEngine.StatisticalAnalyzer do
  ...
  def calculate_correlation(data, fields, %RenderContext{} = context, correlation_type \\ :pearson) do
  ...
  """
  @spec calculate_regression(list(), atom(), atom(), regression_type(), RenderContext.t()) :: map()
  def calculate_regression(data, x_field, y_field, regression_type, %RenderContext{} = context) do
    ...
    case {length(x_values), length(y_values)} do
      {0, _} -> %{error: "No numeric data found for field #{x_field}"}
      {_, 0} -> %{error: "No numeric data found for field #{y_field}"}
      {x_len, y_len} when x_len != y_len -> %{error: "Mismatched data lengths: #{x_len} vs #{y_len}"}
      {len, len} when len < 3 -> %{error: "At least 3 data points required for regression"}
      _ -> perform_regression_analysis(x_values, y_values, regression_type, context)
    end
  end
  ...
  defp calculate_bivariate_correlation(data, [field1, field2], context, correlation_type) do
    ...
    case {length(values1), length(values2)} do
      {0, _} -> %{error: "No numeric data for field #{field1}"}
      {_, 0} -> %{error: "No numeric data for field #{field2}"}
      {len1, len2} when len1 != len2 -> %{error: "Mismatched data lengths"}
      {len, len} when len < 3 -> %{error: "At least 3 data points required"}
      _ -> 
        coefficient = case correlation_type do
  ...
  defp calculate_correlation_matrix(data, fields, context, correlation_type) do
    ...
    correlations = field_combinations
    |> Enum.map(fn [f1, f2] ->
      if f1 == f2 do
        {{f1, f2}, 1.0}  # Perfect self-correlation
      else
  ...
  defp calculate_polynomial_regression(x_values, y_values, context) do
    ...
    if length(x_values) < 4 do
      ...
      %{
        ...
        r_squared: 0.0,  # Placeholder - would need matrix operations for proper calculation
  ...
  defp calculate_exponential_trend(time_series_data, context) do
    ...
    case calculate_exponential_regression(x_values, y_values, context) do
      ...
      regression_result ->
        ...
        predictions = for i <- 1..5 do
          future_x = last_x + i
          future_y = regression_result.coefficient_a * :math.exp(regression_result.coefficient_b * future_x)
          %{period: i, predicted_value: future_y}
        end
  ...
  defp calculate_field_statistics(values, field, context) when is_list(values) and length(values) > 0 do
  ...
  defp time_series_to_numeric(time_series_data) do
    ...
    |> Enum.map(fn {point, index} ->
      case point do
        %{x: x, y: y} when is_number(y) -> {index, y}
  ...
  defp detect_seasonality(time_series_data) do
    ...
    if length(y_values) < 12 do
      ...
      periods_to_check = [12, 4, 7]  # Monthly, quarterly, weekly
      
      best_period = periods_to_check
      |> Enum.map(fn period ->
      ...
      if abs(strength) > 0.3 do  # Threshold for significant seasonality
        {:ok, %{
          period: period,
  ...
  defp calculate_kurtosis(values, mean, std_dev) do
    if std_dev == 0 do
      ...
      n = length(values)
      sum_fourth_deviations = values
      |> Enum.map(fn val -> :math.pow((val - mean) / std_dev, 4) end)
      |> Enum.sum()
      
      (sum_fourth_deviations / n) - 3  # Excess kurtosis
    end
  end
  ...
  defp detect_zscore_outliers(data, field, values, context) do
    ...
    threshold = 2.0  # Standard threshold for z-score outliers
  ...
  defp calculate_correlation_p_value(correlation, sample_size) do
    ...
    if sample_size <= 2 do
      ...
      cond do
        abs_t > 2.576 -> 0.01   # p < 0.01
        abs_t > 1.96 -> 0.05    # p < 0.05
        abs_t > 1.645 -> 0.10   # p < 0.10
        true -> 0.20            # p >= 0.10
      end
    end
  end
