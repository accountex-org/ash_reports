defmodule AshReports.Verifiers.ValidateReports do
  ...
  def verify(dsl_state) do
    ...
    with :ok <- validate_unique_report_names(dsl_state, module),
         ...
         :ok <- validate_driving_resources(dsl_state, module) do
      validate_detail_bands(dsl_state, module)
    end
  end
  ...
  defp validate_driving_resources(dsl_state, module) do
    ...
    Enum.reduce_while(reports, :ok, fn report, :ok ->
      validate_single_driving_resource(report, module)
    end)
  end

  defp validate_single_driving_resource(report, module) do
    cond do
      is_nil(report.driving_resource) ->
        {:cont, :ok}

      is_atom(report.driving_resource) ->
        {:cont, :ok}
