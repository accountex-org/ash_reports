defmodule AshReports.InteractiveEngine do
  ...
  """
  @spec filter(list(), filter_criteria(), RenderContext.t()) :: {:ok, list()} | {:error, String.t()}
  def filter(data, criteria, %RenderContext{} = context) when is_list(data) and is_map(criteria) do
  ...
  def sort(data, sort_specs, %RenderContext{} = context) when is_list(data) and is_list(sort_specs) do
    start_time = System.monotonic_time(:microsecond)
  ...
  def create_pivot_table(data, config, %RenderContext{} = context) when is_list(data) and is_map(config) do
    start_time = System.monotonic_time(:microsecond)
  ...
  def analyze_correlation(data, fields, %RenderContext{} = context) when is_list(data) and is_list(fields) do
    start_time = System.monotonic_time(:microsecond)
  ...
  """
  @spec calculate_regression(list(), atom(), atom(), atom(), RenderContext.t()) :: analysis_result()
  def calculate_regression(data, x_field, y_field, regression_type, %RenderContext{} = context) do
    start_time = System.monotonic_time(:microsecond)
    
    try do
      regression_result = StatisticalAnalyzer.calculate_regression(data, x_field, y_field, regression_type, context)
  ...
  def setup_real_time_stream(stream_config, %RenderContext{} = context) do
    try do
      stream_pid = GenServer.start_link(
  ...
  """
  @spec aggregate(list(), atom() | [atom()], atom(), atom(), RenderContext.t()) :: {:ok, list()} | {:error, String.t()}
  def aggregate(data, group_fields, value_field, aggregation_func, %RenderContext{} = context) do
    start_time = System.monotonic_time(:microsecond)
  ...
  defp locale_compare(str1, str2, locale) do
    # Use locale-aware string comparison when available
    case locale do
      "ar" -> String.compare(str1, str2)  # Could be enhanced with Arabic collation
      "he" -> String.compare(str1, str2)  # Could be enhanced with Hebrew collation
  ...
  defp get_concurrent_operations_count do
    ...
    |> Enum.count(fn pid ->
      case Process.info(pid, :dictionary) do
        {:dictionary, dict} -> 
          Keyword.get(dict, :"$initial_call") == {AshReports.InteractiveEngine.StreamProcessor, :init, 1}
        _ -> false
      end
    end)
  end
