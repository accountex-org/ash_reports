defmodule AshReports.HeexRenderer.ChartTemplates do
  ...
  def filter_dashboard(config, %RenderContext{} = context) do
    template_config = build_filter_template_config(config, context)
    generate_filter_dashboard_html(template_config, context)
  end

  defp build_filter_template_config(config, context) do
    %{
  ...
  defp determine_sidebar_class(sidebar_position) do
    case sidebar_position do
      :left -> "sidebar-left"
      :right -> "sidebar-right"
      _ -> "sidebar-left"
    end
  ...
  defp generate_filter_dashboard_html(template_config, context) do
    """
    <div class="ash-filter-dashboard #{template_config.sidebar_class} #{template_config.rtl_class}">
      
      <div class="dashboard-sidebar">
        <h4 class="sidebar-title">#{get_filters_text(context.locale)}</h4>
        <div class="filter-controls">
          #{template_config.filter_controls}
        </div>
    ...
    tab_headers =
      ...
      |> Enum.map(fn {tab, index} ->
        ...
        <button class="tab-header #{active_class}" 
          ...
          #{tab[:title] || "Chart " <> to_string(index + 1)}
        </button>
        """
      end)
      |> Enum.join("\n")
