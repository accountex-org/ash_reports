defmodule AshReports.ChartEngine.ChartData do
  ...
  @type data_type :: :numeric | :categorical | :coordinate_pairs | :multi_series | :time_series | :mixed
  ...
  def aggregate(%__MODULE__{} = chart_data, aggregation_type) do
    aggregated_data = case aggregation_type do
  ...
  def filter(%__MODULE__{} = chart_data, criteria) when is_map(criteria) do
    filtered_data = chart_data.processed_data
    ...
    %{chart_data |
      ...
      value_count: length(filtered_data)
    }
    |> calculate_statistics()
  end
  ...
  def enable_real_time(%__MODULE__{} = chart_data, opts \\ []) do
    %{chart_data |
      last_updated: DateTime.utc_now(),
  ...
  defp process_map_data(chart_data, data) do
    ...
    |> Enum.map(fn {{series_name, series_data}, idx} ->
      ...
      colors = generate_colors(map_size(data))
    ...
    all_processed = datasets
    |> Enum.flat_map(fn dataset -> dataset.data end)
  ...
  defp detect_time_interval(times) when length(times) >= 2 do
    ...
    case time_diffs do
      ...
      diffs ->
        ...
        cond do
          avg_diff < 3600 -> :minute     # Less than 1 hour
          avg_diff < 86400 -> :hour      # Less than 1 day
          avg_diff < 604800 -> :day      # Less than 1 week
          avg_diff < 2592000 -> :week    # Less than 1 month
          true -> :month
        end
    end
  end
  ...
  defp generate_colors(count) do
    ...
    if count <= length(base_colors) do
      ...
      additional_colors = for i <- length(base_colors)..(count - 1) do
        hue = rem(i * 137, 360)  # Golden angle for good distribution
        "hsl(#{hue}, 70%, 50%)"
      end
