defmodule AshReports.ChartEngine.ChartConfig do
  ...
  def real_time(type, data, opts \\ []) do
    default_realtime_opts = [
      ...
      cache_enabled: false,
      update_interval: 10_000  # 10 seconds default
    ]
  ...
  defp validate_chart_type(type) when type in [:line, :bar, :pie, :area, :scatter, :histogram, :boxplot, :heatmap] do
    :ok
  end
  ...
  defp validate_data_presence(data) when data in [nil, []], do: {:error, "Chart data cannot be empty"}
  ...
  defp validate_real_time_config(%{real_time: true, update_interval: interval}) when interval < 1000 do
