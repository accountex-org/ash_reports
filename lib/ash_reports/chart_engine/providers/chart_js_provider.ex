defmodule AshReports.ChartEngine.Providers.ChartJsProvider do
  ...
  defp build_chartjs_config(chart_spec, %RenderContext{} = context) do
    ...
    config_with_rtl = if context.text_direction == "rtl" do
  ...
  defp build_scales(chart_type, %RenderContext{} = context) when chart_type != :pie do
    ...
    case context.locale do
      locale when locale in ["ar", "fa", "ur"] ->
        put_in(base_scales, [:y, :ticks, :callback], "function(value) { return value.toLocaleString('#{locale}'); }")
  ...
  defp apply_rtl_adaptations(config, %RenderContext{locale: locale}) when locale in ["ar", "he", "fa", "ur"] do
    config
  ...
  defp generate_svg_pie_chart(config, context) do
  ...
  defp generate_svg_bar_chart(config, context) do
    data_values = config.data.datasets |> List.first() |> Map.get(:data, [])
  ...
  defp get_chart_fallback_text(config, %RenderContext{} = context) do
    case context.locale do
