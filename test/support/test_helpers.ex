unless Code.ensure_loaded?(AshReports.TestHelpers) do
  ...
  def parse_dsl(dsl_content, extension \\ AshReports) do
    ...
    module_name = :"TestModule#{:rand.uniform(999999)}"
  ...
  def assert_band_order(bands) do
    type_order = [
      :title, :page_header, :column_header, :group_header, :detail_header,
      :detail, :detail_footer, :group_footer, :column_footer, :page_footer, :summary
    ]
  ...
  def assert_element_types(elements, expected_types) do
    actual_types = Enum.map(elements, fn element ->
      case element do
  ...
  """
  def assert_dsl_error_at_path(dsl_content, expected_error, expected_path, extension \\ AshReports) do
    case parse_dsl(dsl_content, extension) do
      {:ok, _dsl_state} ->
        flunk("Expected DSL to produce error at path #{inspect(expected_path)}, but it was valid")
