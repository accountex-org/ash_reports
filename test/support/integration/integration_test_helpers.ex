defmodule AshReports.Integration.TestHelpers do
  ...
  def create_multilingual_test_data do
    [
      ...
      %Customer{
        id: 3,
        name: "דוד כהן", # David Cohen in Hebrew
  ...
  def create_cldr_context(locale) do
    ...
    metadata = Map.merge(context.locale_metadata, %{
      cldr_enabled: true,
  ...
  def build_rtl_test_report do
    ...
    |> Map.put(:title, "تقرير العملاء") # "Customer Report" in Arabic
  ...
  def consistent_rtl_behavior?(results) do
    ...
    rtl_indicators = Enum.map(results, fn {renderer, result} ->
  ...
  def contains_translations?(result, locale, _renderer) do
    ...
    case locale do
      "ar" -> String.contains?(result, ["العملاء", "التقرير"]) # Arabic text
      "he" -> String.contains?(result, ["לקוחות", "דוח"]) # Hebrew text  
      "es" -> String.contains?(result, ["Clientes", "Informe"]) # Spanish text
      _ -> String.length(result) > 0
    end
  end
  
  def is_valid_output?(result, renderer) do
    case renderer do
  ...
  def contains_fallback_text?(result) do
    ...
    String.contains?(result, ["Name", "Email", "Customer"]) # English fallbacks
  end
  ...
  def create_performance_test_data(size \\ 100) do
    ...
    |> Enum.map(fn i ->
      %Customer{
        ...
        created_at: DateTime.from_naive!(~N[2024-01-01 10:00:00], "Etc/UTC") |> DateTime.add(i, :day)
      }
    end)
  end
