defmodule AshReports.Integration.BenchmarkHelpers do
  ...
  def locale_scalability_benchmark do
    ...
    scenarios = locales
    |> Enum.take(5) # Limit for reasonable benchmark time
    |> Map.new(fn locale ->
      {"locale_#{locale}", fn ->
  ...
  defp validate_memory_usage(scenario) do
    if scenario.memory_usage_data do
      ...
      if memory_mb <= max_memory do
        {:ok, "Memory usage #{Float.round(memory_mb, 2)}MB is within limits (max: #{max_memory}MB)"}
      else
  ...
  defp format_scenario_summary(scenarios) do
    ...
    |> Enum.map(fn scenario ->
      ...
      memory_mb = if scenario.memory_usage_data do
  ...
  defp analyze_performance_trends(scenarios) do
    ...
    if baseline do
      ...
      "Baseline scenario not found for comparison analysis."
    end
  end
  ...
  defp generate_recommendations(scenarios) do
    ...
    slow_scenarios = scenarios
    |> Enum.filter(fn scenario ->
      time_ms = scenario.run_time_data.statistics.average / 1_000_000
      time_ms > 1000 # More than 1 second
    end)
    
    recommendations = if length(slow_scenarios) > 0 do
      ["Consider optimizing slow scenarios: #{Enum.map(slow_scenarios, & &1.name) |> Enum.join(", ")}" | recommendations]
    else
      recommendations
    end
    
    # Check for memory issues
    memory_intensive = scenarios
    |> Enum.filter(fn scenario ->
      if scenario.memory_usage_data do
        memory_mb = scenario.memory_usage_data.statistics.average / (1024 * 1024)
        memory_mb > 25 # More than 25MB
      else
        false
      end
    end)
    
    recommendations = if length(memory_intensive) > 0 do
      ["Review memory usage for scenarios: #{Enum.map(memory_intensive, & &1.name) |> Enum.join(", ")}" | recommendations]
    else
      recommendations
    end
