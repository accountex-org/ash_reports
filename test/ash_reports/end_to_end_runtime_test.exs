defmodule AshReports.EndToEndRuntimeTest do
  ...
  describe "complete runtime execution workflow" do
    test "executes full report generation workflow with real data" do
            ...
            bands do
              band :title do
                elements do
                  ...
                  expression "filter_info", 
                            expression: expr(if(region_filter != nil, 
                ...
                bands do
                      ...
                      band :detail do
                        elements do
                          ...
                          expression :credit_info, 
                                    expression: expr("Credit: $" <> credit_limit <> 
                                                   if(credit_limit >= min_credit, " ✓", " ✗"))
                          expression :position_info, 
                                    expression: expr("Customer #" <> customer_count <> 
                                                   " | Region #" <> region_count)
                        end
                      end
                    end
                  ...
                  band :group_footer, group_level: 2 do
                    elements do
                      ...
                      aggregate :status_credit_avg, function: :avg, source: [:credit_limit], format: :currency
                      
                      expression :status_summary, 
                                expression: expr("Status " <> status <> " Summary: " <> 
                                               status_count <> " customers, " <>
                                               "Total: " <> status_credit_total <> ", " <>
              ...
              band :group_footer, group_level: 1 do
                elements do
                  ...
                  aggregate :region_total_credit, function: :sum, source: [:credit_limit], format: :currency
                  aggregate :region_avg_credit, function: :avg, source: [:credit_limit], format: :currency
                  aggregate :region_min_credit, function: :min, source: [:credit_limit], format: :currency
                  aggregate :region_max_credit, function: :max, source: [:credit_limit], format: :currency
                  
                  expression :region_final_summary, 
                            expression: expr("Region " <> region <> " Totals:")
                  expression :region_stats, 
                            expression: expr("Customers: " <> region_total_count <> 
                                           " | Total Credit: " <> region_total_credit)
                  expression :region_credit_stats, 
                            expression: expr("Credit - Avg: " <> region_avg_credit <> 
                                           " | Min: " <> region_min_credit <> 
                                           " | Max: " <> region_max_credit)
                  expression :region_percentage, 
                            expression: expr("Region represents " <> 
              ...
              band :summary do
                elements do
                  ...
                  expression :final_credit_range, 
                            expression: expr("Credit Range: " <> grand_min_credit <> 
                                           " to " <> grand_max_credit)
                  expression :final_variables, 
                            expression: expr("Variables Used - Count: " <> customer_count <> 
                                           " | Total: " <> total_credit)
      ...
      assert validated_params.report_date == Date.utc_today()  # Default applied
      ...
      assert report_module.Html.supports_streaming?() in [true, false]  # Boolean response
      ...
      edge_params = %{
        region_filter: "",  # Empty string
        min_credit: Decimal.new("0"),  # Zero value
        report_date: ~D[1900-01-01]  # Very old date
      }
    ...
    test "handles complex parameter scenarios in runtime execution" do
              ...
              band :title do
                elements do
                  ...
                  expression "parameter_summary", 
                            expression: expr("Parameters - Multiplier: " <> multiplier <> 
                                           " | Threshold: " <> threshold <> 
                                           " | Details: " <> enable_details)
                  expression "date_range_info", 
                            expression: expr(if(date_range_start != nil && date_range_end != nil,
                              "Date Range: " <> format_date(date_range_start, :short) <> 
              ...
              band :detail do
                elements do
                  ...
                  expression :threshold_check, 
                            expression: expr(if(credit_limit >= threshold, 
                              "Above Threshold ✓", 
                              "Below Threshold ✗"))
                  expression :style_dependent, 
                            expression: expr(case format_style do
                              "detailed" -> name <> " (" <> email <> ") - $" <> credit_limit
                              "compact" -> name <> " - $" <> credit_limit
                              _ -> name
                            end)
                  expression :date_filtered, 
                            expression: expr(if(date_range_start != nil,
      ...
      assert partial_validated.threshold == 5000  # Default
      assert partial_validated.enable_details == true
      assert partial_validated.format_style == "standard"  # Default
      
      # Test parameter edge cases
      edge_params = %{
        title_override: "",  # Empty string
        multiplier: Decimal.new("0"),  # Zero multiplier
        threshold: 0,  # Zero threshold
        enable_details: false,
        format_style: "unknown"  # Unknown style
      }
    ...
    test "validates variable behavior in runtime execution context" do
        ...
        reports do
          report :variable_runtime_test do
            ...
            variables do
              ...
              variable :global_credit_sum, type: :sum, expression: expr(credit_limit), reset_on: :report
              variable :region_credit_sum, type: :sum, expression: expr(credit_limit), 
                       reset_on: :group, reset_group: 1
              variable :running_average, type: :avg, expression: expr(credit_limit), reset_on: :report
              variable :high_credit_count, type: :count_where, 
                       condition: expr(credit_limit > 5000), reset_on: :report
            end
            ...
            bands do
              band :title do
                elements do
                  ...
                  expression "global_info", 
                            expression: expr("Global Counter: " <> global_counter <> 
                                           " | Global Credit Sum: $" <> global_credit_sum)
                  expression "analysis_info", 
                            expression: expr("Running Average: $" <> running_average <> 
              ...
              band :group_header, group_level: 1 do
                elements do
                  field :region_name, source: [:region]
                  expression "region_progress", 
                            expression: expr("Region Progress - Local: " <> region_counter <> 
                                           " | Global: " <> global_counter)
                  expression "region_credit_progress", 
                            expression: expr("Region Credit: $" <> region_credit_sum <> 
                  ...
                  band :group_header, group_level: 2 do
                    elements do
                      ...
                      expression "status_progress", 
                                expression: expr("Status: " <> status_counter <> 
                      ...
                      band :detail do
                        elements do
                          ...
                          expression :position_tracking, 
                                    expression: expr("Position - Status: #" <> status_counter <> 
                                                   " | Region: #" <> region_counter <> 
                                                   " | Global: #" <> global_counter)
                          expression :running_stats, 
                                    expression: expr("Running Avg: $" <> running_average <> 
                                                   " | This Credit: $" <> credit_limit)
                          expression :credit_analysis, 
                                    expression: expr(if(credit_limit > 5000, 
                  ...
                  band :group_footer, group_level: 2 do
                    elements do
                      ...
                      expression "variable_vs_aggregate", 
                                expression: expr("Variable Count: " <> status_counter <> 
              ...
              band :group_footer, group_level: 1 do
                elements do
                  ...
                  expression "region_counts", 
                            expression: expr("Region Count: " <> region_counter <> 
              ...
              band :summary do
                elements do
                  ...
                  expression "final_credit_stats", 
                            expression: expr("Total Credit: $" <> global_credit_sum <> 
                                           " | Average: $" <> running_average)
                  expression "final_analysis", 
                            expression: expr("High Credit Customers: " <> high_credit_count <> 
                                           " (" <> (high_credit_count / global_counter * 100) <> "%)")
                  
                  aggregate :final_aggregate_count, function: :count, source: [:id]
                  aggregate :final_aggregate_sum, function: :sum, source: [:credit_limit], format: :currency
                  
                  expression "variable_aggregate_validation", 
                            expression: expr("Validation - Variable Count: " <> global_counter <> 
                                           " | Aggregate Count: " <> final_aggregate_count <> 
                                           " | Variable Sum: $" <> global_credit_sum <> 
      ...
      assert length(report_vars) == 4  # global_counter, global_credit_sum, running_average, high_credit_count
      assert length(group_vars) == 3   # region_counter, status_counter, region_credit_sum
        ...
        reports do
          report :grouping_sorting_runtime_test do
            ...
            groups do
              ...
              group :status, field: [:status], sort: :desc
              group :credit_tier, field: expr(if(group_by_credit,
                case do
                ...
                "All"
              )), sort: :asc, enabled: expr(group_by_credit)
            end
              ...
              band :title do
                elements do
                  ...
                  expression "grouping_info", 
                            expression: expr(if(group_by_credit, 
              ...
              band :group_header, group_level: 1 do
                ...
                bands do
                  band :group_header, group_level: 2 do
                    ...
                    bands do
                      if true do  # Conditional band structure based on grouping
              ...
              band :summary do
                elements do
                  ...
                  expression "sorting_summary", 
                            expression: expr("Total customers: " <> total_count <> 
              ...
              band :title do
                elements do
                  ...
                  expression "metadata_info", 
                            expression: expr(if(include_metadata, 
                              "Metadata included", 
                              "Metadata excluded")),
              ...
              band :detail do
                elements do
                  ...
                  expression :format_specific_content, 
                            expression: expr(case output_format do
              ...
              band :summary do
                elements do
                  ...
                  expression "format_summary", 
                            expression: expr("Report completed in " <> output_format <> 
  ...
  describe "error handling and edge cases in runtime" do
    test "handles runtime parameter validation errors gracefully" do
      ...
      type_errors = %{
        ...
        positive_integer: "not_an_integer",  # Should be integer
        valid_date: "invalid_date",  # Should be date
        decimal_value: "not_a_decimal"  # Should be decimal
      }
      
      assert {:error, type_error_list} = report_module.validate_params(type_errors)
      assert is_list(type_error_list)
      assert length(type_error_list) >= 3  # At least 3 type errors
      
      # Test edge case values
      edge_cases = %{
        required_string: "",  # Empty string
        positive_integer: 0,  # Zero
        valid_date: ~D[1900-01-01],  # Very old date
        decimal_value: Decimal.new("0.000000001")  # Very small decimal
      }
    ...
    test "handles query building edge cases" do
              ...
              band :detail do
                elements do
                  ...
                  expression "filtered_info", 
                            expression: expr(if(filter_value != nil,
      ...
      extreme_params = %{
        filter_value: String.duplicate("x", 1000),  # Very long string
        numeric_filter: Decimal.new("999999999999.99")  # Very large number
      }
