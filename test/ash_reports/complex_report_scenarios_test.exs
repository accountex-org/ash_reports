defmodule AshReports.ComplexReportScenariosTest do
  ...
  describe "multi-band reports with all band types" do
    test "creates report with complete band hierarchy" do
            ...
            bands do
              ...
              band :region_group_header, type: :group_header, group_level: 1 do
                elements do
                  ...
                  box "region_highlight", from: [x: 0, y: 0], to: [x: 600, y: 25], 
                ...
                bands do
                    ...
                    bands do
                      ...
                      band :detail do
                        elements do
                          ...
                          field :credit_limit, source: [:credit_limit], format: :currency, position: [x: 400, y: 0]
                          expression :full_customer_info, 
                                    expression: expr(name <> " (" <> email <> ") - " <> status),
                                    position: [x: 0, y: 15]
                          line "detail_separator", from: [x: 0, y: 25], to: [x: 600, y: 25], 
                      ...
                      band :detail_footer do
                        elements do
                          ...
                          expression "detail_info", 
                                    expression: expr("Processed " <> detail_record_count <> " detail records")
                        end
                      end
                    end
                  end
                  ...
                  band :status_group_footer, type: :group_footer, group_level: 2 do
                    elements do
                      ...
                      aggregate :status_credit_total, function: :sum, source: [:credit_limit], format: :currency
                      aggregate :status_credit_average, function: :avg, source: [:credit_limit], format: :currency
                      expression :status_summary, 
                                expression: expr("Status " <> status <> ": " <> status_customer_count <> " customers, " <> 
                                               "Total Credit: " <> status_credit_total <> ", " <>
                                               "Average: " <> status_credit_average)
                      box "status_footer_box", from: [x: 20, y: 0], to: [x: 580, y: 40],
              ...
              band :region_group_footer, type: :group_footer, group_level: 1 do
                elements do
                  ...
                  aggregate :region_credit_total, function: :sum, source: [:credit_limit], format: :currency
                  aggregate :region_credit_min, function: :min, source: [:credit_limit], format: :currency
                  aggregate :region_credit_max, function: :max, source: [:credit_limit], format: :currency
                  expression :region_summary, 
                            expression: expr("Region " <> region <> " Totals: " <> region_customer_count <> " customers"),
                            position: [x: 0, y: 0]
                  expression :region_credit_range, 
                            expression: expr("Credit range: " <> region_credit_min <> " to " <> region_credit_max),
                            position: [x: 0, y: 15]
                  box "region_footer_box", from: [x: 0, y: 0], to: [x: 600, y: 50],
                      style: [fill_color: "#f0f8ff", border_width: 2]
                  line "region_total_line", from: [x: 0, y: 50], to: [x: 600, y: 50], 
              ...
              band :column_footer do
                elements do
                  ...
                  box "column_footer_box", from: [x: 0, y: 0], to: [x: 600, y: 25],
              ...
              band :summary do
                elements do
                  ...
                  aggregate :grand_total_credit, function: :sum, source: [:credit_limit], format: :currency
                  aggregate :grand_average_credit, function: :avg, source: [:credit_limit], format: :currency
                  ...
                  expression :summary_line3, 
                            ...
                            position: [x: 0, y: 50]
                  box "summary_box", from: [x: 0, y: 0], to: [x: 600, y: 70],
                      style: [fill_color: "#e8f5e8", border_width: 2, border_color: "#4caf50"]
                  line "final_line", from: [x: 0, y: 75], to: [x: 600, y: 75],
                       style: [color: "#4caf50", width: 3]
                  image "summary_icon", source: "/assets/summary_icon.png", 
                        position: [x: 550, y: 10], size: [width: 40, height: 40]
                end
              end
            end
          end
      ...
      expected_types = [
        :title, :page_header, :column_header, :group_header, :group_footer, 
      ...
      assert length(definition.bands) == 8  # All top-level bands
      ...
      assert length(region_header.bands) == 2  # status_group_header and status_group_footer
      ...
      assert length(status_header.bands) == 3  # detail_header, detail, detail_footer
      
      # Verify element counts in key bands
      title_band = Enum.find(definition.bands, &(&1.type == :title))
      assert length(title_band.elements) == 4  # label, image, line, expression
      
      detail_band = Enum.find(status_header.bands, &(&1.type == :detail))
      assert length(detail_band.elements) == 6  # 4 fields, 1 expression, 1 line
      
      summary_band = Enum.find(definition.bands, &(&1.type == :summary))
      assert length(summary_band.elements) == 9  # 1 label, 3 aggregates, 3 expressions, 1 box, 1 line, 1 image
    end
  ...
  describe "reports with all element types and combinations" do
    test "creates report with comprehensive element types" do
            ...
            bands do
              band :title do
                elements do
                  ...
                  label "complex_label", text: expr("Report for " <> (region_filter || "All") <> " - " <> format_date(now(), :short))
                  
                  # All image variations
                  image "static_image", source: "/assets/logo.png"
                  image "conditional_image", source: expr(if(show_images, "/assets/full_logo.png", "/assets/text_logo.png"))
                  image "positioned_image", source: "/assets/header.png", 
                  ...
                  line "styled_line", from: [x: 0, y: 65], to: [x: 600, y: 65],
                       style: [color: "#ff6600", width: 3, dash_pattern: [5, 3]]
                  
                  # All box variations
                  box "simple_box", from: [x: 0, y: 70], to: [x: 200, y: 120]
                  box "styled_box", from: [x: 220, y: 70], to: [x: 420, y: 120],
                      style: [border_width: 2, border_color: "#0066cc", fill_color: "#e6f3ff"]
                  box "complex_box", from: [x: 440, y: 70], to: [x: 600, y: 120],
                      style: [border_width: 1, border_color: "#333333", fill_color: "#f9f9f9", 
                              corner_radius: 5, shadow: true]
                  ...
                  expression :complex_expr, 
                            expression: expr("Report: " <> (region_filter || "All") <> " | Generated: " <> 
              ...
              band :group_header, group_level: 1 do
                elements do
                  ...
                  field :styled_field, source: [:status], 
                        style: [font_weight: :bold, color: "#0066cc"]
                  field :conditional_field, source: [:email], 
                        visible: expr(region_filter == nil || region == region_filter)
                  
                  # Field with complex formatting
                  field :complex_formatted_field, source: [:credit_limit], 
                  ...
                  expression :field_calculation, 
                            expression: expr("Customer: " <> name <> " (Credit: " <> currency_symbol <> credit_limit <> ")")
                  
                  # Aggregate previews
                  aggregate :region_preview_count, function: :count, source: [:id], scope: :current_group
                ...
                bands do
                  band :detail do
                    elements do
                      ...
                      field :credit_decimal, source: [:credit_limit], format: {:decimal, precision: 2}
                      field :credit_percentage, source: [:credit_limit], format: {:percentage, precision: 1}
                      
                      # Complex expressions combining fields
                      expression :full_customer_info, 
                                expression: expr(id <> ": " <> name <> " (" <> email <> ")")
                      expression :status_info, 
                                expression: expr(region <> " | " <> status <> " | " <> currency_symbol <> credit_limit)
                      expression :conditional_status, 
                                expression: expr(case status do
                      ...
                      box "detail_highlight", from: [x: 0, y: 0], to: [x: 10, y: 25],
                          style: [fill_color: expr(case status do
                            :active -> "#4caf50"
                            :inactive -> "#f44336"
                            _ -> "#ff9800"
                          end)]
              ...
              band :group_footer, group_level: 1 do
                elements do
                  ...
                  aggregate :active_count, function: :count_where, 
                           condition: expr(status == :active)
                  aggregate :inactive_count, function: :count_where, 
                           condition: expr(status == :inactive)
                  aggregate :high_credit_sum, function: :sum_where, 
                           source: [:credit_limit], condition: expr(credit_limit > 5000), format: :currency
                  
                  # Complex expressions using aggregates
                  expression :summary_stats, 
                            expression: expr("Count: " <> count_agg <> " | Average: " <> currency_symbol <> avg_agg)
                  expression :range_info, 
                            expression: expr("Range: " <> currency_symbol <> min_agg <> " to " <> currency_symbol <> max_agg)
                  expression :activity_ratio, 
                            expression: expr("Active: " <> active_count <> "/" <> count_agg <> 
                                           " (" <> (active_count / count_agg * 100) <> "%)")
                  
                  # Visual summary elements
                  box "summary_box", from: [x: 0, y: 0], to: [x: 600, y: 60],
                      style: [fill_color: "#f5f5f5", border_width: 1, border_color: "#cccccc"]
                  line "summary_top_line", from: [x: 0, y: 0], to: [x: 600, y: 0],
                       style: [color: "#0066cc", width: 2]
                  line "summary_bottom_line", from: [x: 0, y: 60], to: [x: 600, y: 60],
                       style: [color: "#0066cc", width: 2]
                end
              end
              ...
              band :summary do
                elements do
                  ...
                  aggregate :grand_total_customers, function: :count, source: [:id]
                  aggregate :grand_total_credit, function: :sum, source: [:credit_limit], format: :currency
                  aggregate :grand_avg_credit, function: :avg, source: [:credit_limit], format: :currency
                  aggregate :grand_min_credit, function: :min, source: [:credit_limit], format: :currency
                  aggregate :grand_max_credit, function: :max, source: [:credit_limit], format: :currency
                  ...
                  expression :grand_summary_line4, 
                            expression: expr("Credit Range: " <> currency_symbol <> grand_min_credit <> 
                                           " to " <> currency_symbol <> grand_max_credit)
                  
                  # Final visual elements
                  box "final_summary_box", from: [x: 0, y: 0], to: [x: 600, y: 80],
                      style: [fill_color: "#e8f5e8", border_width: 3, border_color: "#4caf50"]
                  image "completion_icon", source: "/assets/complete_icon.png",
                        position: [x: 550, y: 10], size: [width: 40, height: 40]
                  line "final_line", from: [x: 0, y: 85], to: [x: 600, y: 85],
      ...
      all_elements = Enum.flat_map(definition.bands, fn band ->
        Enum.flat_map([band | extract_nested_bands(band)], & &1.elements)
      end)
      
      element_type_counts = Enum.reduce(all_elements, %{}, fn element, acc ->
        type = case element do
            ...
            bands do
              band :detail do
                elements do
                  ...
                  line "styled_line", from: [x: 0, y: 45], to: [x: 500, y: 45],
                       style: [color: "#00cc66", width: 3, dash_pattern: [10, 5, 3, 5]]
                  
                  # Test image with positioning and sizing
                  image "positioned_image", source: "/assets/test.png",
                        position: [x: 450, y: 0], size: [width: 50, height: 20]
                end
              end
            end
          end
  ...
  describe "reports with variables and grouping scenarios" do
    test "creates report with comprehensive variable types" do
        ...
        reports do
          report :comprehensive_variables do
            ...
            variables do
              ...
              # Sum variables
              variable :total_credit, type: :sum, expression: expr(credit_limit), reset_on: :report
              variable :group_credit, type: :sum, expression: expr(credit_limit), reset_on: :group, reset_group: 1
              variable :subgroup_credit, type: :sum, expression: expr(credit_limit), reset_on: :group, reset_group: 2
              
              # Average variables
              variable :running_avg_credit, type: :avg, expression: expr(credit_limit), reset_on: :report
              variable :group_avg_credit, type: :avg, expression: expr(credit_limit), reset_on: :group, reset_group: 1
              ...
              variable :group_min_credit, type: :min, expression: expr(credit_limit), reset_on: :group, reset_group: 1
              variable :group_max_credit, type: :max, expression: expr(credit_limit), reset_on: :group, reset_group: 1
              
              # Conditional count variables
              variable :active_customers, type: :count_where, 
                       condition: expr(status == :active), reset_on: :report
              variable :inactive_customers, type: :count_where, 
                       condition: expr(status == :inactive), reset_on: :report
              variable :high_credit_customers, type: :count_where, 
                       condition: expr(credit_limit > 5000), reset_on: :group, reset_group: 1
              
              # Conditional sum variables
              variable :active_credit_sum, type: :sum_where, 
                       expression: expr(credit_limit), condition: expr(status == :active), reset_on: :report
              variable :high_value_sum, type: :sum_where, 
                       expression: expr(credit_limit), condition: expr(credit_limit > 10000), reset_on: :group, reset_group: 1
              
              # Complex expression variables
              variable :credit_variance, type: :expression, 
                       expression: expr((group_max_credit - group_min_credit) / group_avg_credit), 
                       reset_on: :group, reset_group: 1
              variable :activity_ratio, type: :expression, 
                       expression: expr(active_customers / total_customers * 100), reset_on: :report
              variable :group_percentage, type: :expression, 
              ...
              band :title do
                elements do
                  ...
                  expression :report_totals, 
                            expression: expr("Report Scope: " <> total_customers <> " customers, $" <> total_credit)
                  expression :activity_summary, 
                            expression: expr("Active: " <> active_customers <> " (" <> activity_ratio <> "%), " <>
              ...
              band :group_header, group_level: 1 do
                elements do
                  ...
                  expression :group_intro, 
                            expression: expr("Region: " <> region <> " (" <> group_percentage <> "% of total)")
                  expression :group_stats, 
                            expression: expr("Customers: " <> group_customers <> ", Credit: $" <> group_credit)
                  expression :group_averages, 
                            expression: expr("Average: $" <> group_avg_credit <> ", Range: $" <> 
                ...
                bands do
                  band :group_header, group_level: 2 do
                    elements do
                      ...
                      expression :subgroup_info, 
                                expression: expr("Status: " <> status <> " - " <> subgroup_customers <> " customers")
                      ...
                      band :group_footer, group_level: 3 do
                        elements do
                          expression :tier_summary, 
                                    expression: expr("Credit Tier Summary - Count: " <> subgroup_customers <> 
                  ...
                  band :group_footer, group_level: 2 do
                    elements do
                      expression :status_summary, 
                                expression: expr("Status Group: " <> status <> " - " <> subgroup_customers <> 
                                               " customers, $" <> subgroup_credit <> " total credit")
                      expression :status_vs_region, 
                                expression: expr("Percentage of region: " <> 
              ...
              band :group_footer, group_level: 1 do
                elements do
                  ...
                  expression :region_totals, 
                            expression: expr("Total: " <> group_customers <> " customers, $" <> group_credit)
                  expression :region_averages, 
                            expression: expr("Average: $" <> group_avg_credit <> 
                                           ", Min: $" <> group_min_credit <> 
                                           ", Max: $" <> group_max_credit)
                  expression :region_high_value, 
                            expression: expr("High Credit (>$5000): " <> high_credit_customers <> 
                                           " customers, $" <> high_value_sum <> " total")
                  expression :region_percentage, 
                            expression: expr("Region represents " <> group_percentage <> "% of total report")
                end
              end
              ...
              band :summary do
                elements do
                  ...
                  expression :customer_totals, 
                            expression: expr("Customers: " <> total_customers <> 
                                           " (Active: " <> active_customers <> 
                                           ", Inactive: " <> inactive_customers <> ")")
                  expression :credit_totals, 
                            expression: expr("Total Credit: $" <> total_credit <> 
                                           " (Active: $" <> active_credit_sum <> ")")
                  expression :credit_statistics, 
                            expression: expr("Credit Stats - Average: $" <> running_avg_credit <> 
                                           ", Min: $" <> min_credit <> 
                                           ", Max: $" <> max_credit)
                  expression :activity_statistics, 
                            expression: expr("Activity Rate: " <> activity_ratio <> 
      ...
      assert length(definition.variables) == 21  # All defined variables
      ...
      assert length(definition.groups) == 3  # region, status, credit_tier
      ...
      referenced_variables = Enum.filter(variable_names, fn var_name ->
              ...
              band :group_header, group_level: 1 do
                elements do
                  ...
                  expression :group1_scope, 
                            expression: expr("Group 1 Counter: " <> group1_counter <> 
                  ...
                  band :group_header, group_level: 2 do
                    elements do
                      ...
                      expression :group2_scope, 
                                expression: expr("Group 2 Counter: " <> group2_counter <> 
                                               " | Group 1 Counter: " <> group1_counter <> 
                                               " | Report Counter: " <> report_counter)
                    end
                      ...
                      band :detail do
                        elements do
                          ...
                          expression :detail_counters, 
                                    expression: expr("Detail - G2: " <> group2_counter <> 
                                                   " | G1: " <> group1_counter <> 
  ...
  describe "parameter validation and substitution scenarios" do
    test "creates report with comprehensive parameter types and validation" do
            ...
            bands do
              band :title do
                elements do
                  ...
                  label "date_label", text: expr("Report Date: " <> format_date(report_date, :long))
                  
                  # Conditional parameter usage
                  label "filter_info", text: expr(if(region_filter != "All", 
                    "Filtered by Region: " <> region_filter, 
                    "Showing All Regions"))
                  
                  # Complex parameter combinations
                  expression :title_summary, 
                            expression: expr(report_title <> " - " <> format_date(report_date, :short) <> 
                                           " (Generated: " <> format_date(now(), :short) <> ")")
                  
                  # Date range handling
                  expression :date_range, 
                            expression: expr(if(start_date != nil && end_date != nil,
                              "Period: " <> format_date(start_date, :short) <> " to " <> format_date(end_date, :short),
                              "No date range specified"))
                  
                  # Numeric parameter usage
                  expression :credit_filter_info, 
                            expression: expr(if(max_credit_limit != nil,
                              "Credit Limit: " <> currency_symbol <> min_credit_limit <> " to " <> currency_symbol <> max_credit_limit,
              ...
              band :detail do
                elements do
                  ...
                  field :credit_limit, source: [:credit_limit], 
                        format: {:currency, symbol: expr(currency_symbol), precision: expr(precision_digits)}
                  
                  # Conditional elements based on boolean parameters
                  expression :detail_info, 
                            expression: expr(if(show_details, 
                  ...
                  expression :search_highlight, 
                            expression: expr(if(customer_name_search != nil && 
                  ...
                  box "highlight_box", from: [x: 0, y: 0], to: [x: 500, y: 25],
                      style: expr(case format_style do
              ...
              band :summary do
                elements do
                  ...
                  expression :parameter_summary, 
                            expression: expr("Report Parameters: Title='" <> report_title <> 
                                           "', Date=" <> format_date(report_date, :short) <> 
                                           ", Region=" <> region_filter)
                  
                  expression :filter_summary, 
                            expression: expr("Filters Applied: " <> 
                                           (if(customer_name_search != nil, "Name='" <> customer_name_search <> "' ", "")) <>
                  ...
                  expression :display_options, 
                            expression: expr("Display Options: Details=" <> show_details <> 
                  ...
                  aggregate :filtered_credit_sum, function: :sum_where, 
                           source: [:credit_limit], 
                           condition: expr(credit_limit >= min_credit_limit && 
      ...
      assert length(definition.parameters) == 13  # All defined parameters
      ...
      assert length(required_params) == 2  # report_title, report_date
      ...
      assert length(errors) >= 2  # Missing required + type errors
    end
    ...
    test "validates parameter usage in complex expressions" do
              ...
              band :detail do
                elements do
                  ...
                  expression :complex_calculation, 
                            expression: expr(if(enable_calculations && credit_limit > 0,
      ...
      detail_band = hd(definition.bands)
      expressions = Enum.filter(detail_band.elements, fn element ->
  ...
  defp extract_all_expressions(bands) do
    Enum.flat_map(bands, fn band ->
      band_expressions = Enum.flat_map(band.elements, fn element ->
        case element do
      ...
      nested_expressions = case band.bands do
        nil -> []
        nested_bands -> extract_all_expressions(nested_bands)
      end
      
      band_expressions ++ nested_expressions
    end)
  end
