defmodule AshReports.DslCompilationIntegrationTest do
  ...
  describe "full domain compilation with complex reports" do
    test "compiles domain with multiple complex reports successfully" do
        ...
        reports do
          ...
          report :order_analysis_report do
            ...
            variables do
              ...
              variable(:revenue_sum,
                type: :sum,
                expression: expr(total_amount),
                reset_on: :report
              )

              variable(:shipping_sum,
                type: :sum,
                expression: expr(shipping_cost),
                reset_on: :report
              )
            end
      ...
      # title, page_header, column_header, group_header, group_footer, column_footer, page_footer, summary
      assert length(customer_def.bands) == 7
      ...
      assert length(order_def.bands) == 4
      ...
      # title, group_header, group_footer, summary
      assert length(product_def.bands) == 4
    end
    ...
    test "handles deeply nested band structures during compilation" do
                      ...
                      band :level3_header, type: :group_header, group_level: 3 do
                        ...
                        bands do
                          band :level4_header, type: :group_header, group_level: 4 do
                            elements do
                              expression(:month,
                                expression: expr(format_date(created_at, :month))
                              )
                            end
      ...
      # title, level1_header, level1_footer, summary
      assert length(definition.bands) == 3

      # Verify nested band structure
      level1_header = Enum.find(definition.bands, &(&1.name == :level1_header))
      # level2_header, level2_footer
      assert length(level1_header.bands) == 2

      level2_header = Enum.find(level1_header.bands, &(&1.name == :level2_header))
      # level3_header, level3_footer
      assert length(level2_header.bands) == 2

      level3_header = Enum.find(level2_header.bands, &(&1.name == :level3_header))
      # level4_header, level4_footer
      assert length(level3_header.bands) == 2

      level4_header = Enum.find(level3_header.bands, &(&1.name == :level4_header))
      # detail
      assert length(level4_header.bands) == 1
    end
    ...
    test "compiles reports with all element types successfully" do
              ...
              band :comprehensive_band do
                elements do
                  ...
                  expression(:complex_expr,
                    expression:
                      expr(
                        "Customer: " <> name <> " (Credit: $" <> to_string(credit_limit) <> ")"
                      )
                  )
      ...
      element_types =
        Enum.map(band.elements, fn element ->
          case element do
  ...
  describe "generated module verification" do
    ...
    test "verifies parameter validation works correctly" do
      ...
      type_invalid_params = %{
        ...
        required_string: 123,
        # Should be date
        required_date: "not_a_date",
        # Should be integer
        optional_integer: "not_an_integer"
      }
  ...
  describe "compilation edge cases" do
    ...
    test "handles reports with minimal configuration" do
      ...
      # Default format
      assert definition.formats == [:html]
