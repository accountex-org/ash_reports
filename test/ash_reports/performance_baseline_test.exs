defmodule AshReports.PerformanceBaselineTest do
  ...
  # 30 seconds for performance tests
  @performance_timeout 30_000
  # 50MB memory threshold
  @memory_threshold 50_000_000
  # 5 seconds compilation threshold
  @compilation_time_threshold 5_000
  ...
  describe "DSL compilation performance benchmarks" do
    ...
    test "benchmarks simple report compilation time" do
      ...
      if File.exists?(baseline_file) do
        ...
        # 50% increase considered regression
        regression_threshold = previous_baseline * 1.5
    ...
    test "benchmarks complex report compilation time" do
      # Fewer samples for complex reports
      compilation_times =
        for _i <- 1..5 do
          {time_microseconds, _result} =
            measure_time(fn ->
      ...
      # 3x threshold for complex reports
      complex_threshold = @compilation_time_threshold * 3
    ...
    test "benchmarks multiple report compilation scaling" do
      ...
      for {count, time} <- scaling_results do
        ...
        IO.puts(
          "#{count} reports: #{Float.round(time, 2)}ms total, #{Float.round(time_per_report, 2)}ms per report"
        )
      end
      ...
      # 2x allowance for overhead
      expected_max_time = single_time * max_count * 2
  ...
  describe "memory usage benchmarks" do
    ...
    test "measures memory usage during transformer execution" do
      ...
      # 10x threshold for total memory
      assert current_memory < @memory_threshold * 10,
  ...
  describe "generated module efficiency benchmarks" do
    ...
    test "measures generated module size and load time" do
      ...
      # 100ms load time threshold
      assert avg_load_time < 100,
             "Module load time #{avg_load_time}ms exceeds 100ms threshold"

      # 1ms function call threshold
      assert avg_call_time_ms < 1,
    ...
    test "measures parameter validation performance" do
      ...
      invalid_params = %{
        ...
        string_param: 123,
        # Wrong type
        date_param: "not_a_date"
      }

      error_validation_times =
        for _i <- 1..100 do
          {time_microseconds, _result} =
            measure_time(fn ->
      ...
      avg_error_validation_time =
        Enum.sum(error_validation_times) / length(error_validation_times)

      IO.puts("Average error validation time: #{Float.round(avg_error_validation_time, 3)}ms")

      # Assertions for reasonable performance
      # 10ms validation threshold
      assert avg_validation_time < 10,
             "Parameter validation time #{avg_validation_time}ms exceeds 10ms threshold"

      # 20ms error validation threshold
      assert avg_error_validation_time < 20,
  ...
  describe "scaling and stress tests" do
    ...
    test "stress tests compilation with deep nesting" do
      ...
      # 3x allowance for nesting
      expected_max_time = min_time * (max_level / min_level) * 3
      ...
      # 150MB threshold for deep nesting stress test
