defmodule AshReports.Integration.MultiRendererConsistencyTest do
  ...
  describe "RTL Support Consistency" do
    ...
    test "RTL layout calculations consistent across visual renderers" do
      ...
      visual_results = Enum.map(visual_renderers, fn renderer ->
        context = TestHelpers.create_rtl_context("ar")
  ...
  describe "Translation Integration Consistency" do
    ...
    test "translation fallbacks work consistently" do
      ...
      results = Enum.map(@renderers, fn renderer ->
        context = TestHelpers.create_context_with_missing_translations(invalid_locale)
        |> Map.put(:renderer, renderer)
  ...
  describe "CLDR Formatting Consistency" do  
    test "number formatting consistent across renderers" do
      test_numbers = [1234.56, 9876543.21, 0.123, 99999999.99]
      locales = ["en", "ar", "de", "fr"]
      
      for locale <- locales do
        for number <- test_numbers do
          results = Enum.map(@renderers, fn renderer ->
            context = TestHelpers.create_cldr_context(locale)
            |> Map.put(:renderer, renderer)
  ...
  describe "Format Specification Consistency" do
    test "custom format specifications applied consistently" do
      ...
      for format_spec <- format_specs do
        results = Enum.map(@renderers, fn renderer ->
          context = TestHelpers.create_integration_context("en", format_spec)
          |> Map.put(:renderer, renderer)
        ...
        case format_spec.type do
          ...
          :decimal ->
            if format_spec.precision == 3 do
              Enum.each(formatted_values, &assert(String.contains?(&1, ".") and String.length(String.split(&1, ".") |> List.last()) == 3))
            end
          :percentage ->
            Enum.each(formatted_values, &assert(String.contains?(&1, "%")))
        end
  ...
  describe "Error Handling Consistency" do
    test "error scenarios handled consistently across renderers" do
      error_scenarios = [
        %{type: :invalid_locale, context_fn: &TestHelpers.create_context_with_missing_translations/0},
        %{type: :invalid_rtl_config, context_fn: &TestHelpers.create_context_with_invalid_rtl_config/0},
        %{type: :missing_format_spec, context_fn: fn -> TestHelpers.create_integration_context("en", nil) end}
      ]
      ...
      for scenario <- error_scenarios do
        ...
        error_types = Enum.map(results, fn {_, result} ->
          case result do
            {:ok, _} -> :success
            {:error, _} -> :error
            {:exit, _} -> :exit
  ...
  defp extract_css_direction(html_output) do
    if String.contains?(html_output, "dir=\"rtl\"") or String.contains?(html_output, "direction: rtl") do
      "rtl"
    else
      "ltr"
    end
