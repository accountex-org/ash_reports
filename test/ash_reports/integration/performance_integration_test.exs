defmodule AshReports.Integration.PerformanceIntegrationTest do
  ...
  alias AshReports.Integration.{TestHelpers, BenchmarkHelpers}
  ...
  describe "Phase 4 Performance Benchmarks" do
    ...
    test "data size scalability benchmark" do
      ...
      scenarios = results.scenarios
      time_by_size = scenarios
      |> Enum.map(fn scenario ->
        size = case scenario.name do
          "small_dataset_10" -> 10
      ...
      [{small_size, small_time}, {medium_size, medium_time}, {large_size, large_time}] = time_by_size
      ...
      IO.puts("  Medium (#{medium_size}): #{Float.round(medium_time, 2)}ms (#{Float.round(small_to_medium_ratio, 2)}x)")
      IO.puts("  Large (#{large_size}): #{Float.round(large_time, 2)}ms (#{Float.round(medium_to_large_ratio, 2)}x)")
    end
  end
  ...
  describe "Phase 4 Component Performance" do
    ...
    test "CLDR formatting performance impact" do
      ...
      if baseline && cldr_number do
        ratio = cldr_number.run_time_data.statistics.average / baseline.run_time_data.statistics.average
        assert ratio <= 2.0, "CLDR number formatting too slow: #{Float.round(ratio, 2)}x vs baseline"
      end
    end
    ...
    test "translation system performance" do
      ...
      results = BenchmarkHelpers.run_benchmark(scenarios, "tmp/translation_component_benchmarks.html")
      ...
      if direct && translation do
        ratio = translation.run_time_data.statistics.average / direct.run_time_data.statistics.average
        assert ratio <= 5.0, "Translation processing too slow: #{Float.round(ratio, 2)}x vs direct"
      end
    end
  end
  ...
  describe "Memory Usage Profiling" do
    ...
    test "Phase 4 memory usage patterns" do
      ...
      memory_results = Enum.map(test_scenarios, fn scenario ->
        {result, memory_used} = TestHelpers.measure_memory_usage(fn ->
  ...
  describe "Concurrent Performance" do
    ...
    test "concurrent rendering performance" do
      ...
      concurrent_time = :timer.tc(fn ->
        contexts
        |> Task.async_stream(fn context ->
          data = TestHelpers.create_multilingual_test_data()
          AshReports.HtmlRenderer.render_with_context(%{context | report: report, data: data})
